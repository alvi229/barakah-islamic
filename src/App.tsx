...
const NamesOfAllahSection = () => {
  ...
  useEffect(() => {
    fetch('/api/names').then(res => res.json()).then(setNames);
  }, []);
...
const AdminPanel = () => {
  ...
  const handleAnswer = async (id: number) => {
    ...
    if (!answer) return;
    await fetch('/api/admin/qa/answer', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/admin/sections/items', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteItem = async (id: number) => {
    await fetch(`/api/admin/sections/items/${id}`, { method: 'DELETE' });
    fetchAdminData();
  };
  ...
  const handleAddDaily = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/admin/daily-content', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleActivateDaily = async (id: number) => {
    await fetch('/api/admin/daily-content/activate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddTasbih = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/admin/tasbih-presets', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(newTasbih)
    });
  ...
  const handleDeleteTasbih = async (id: number) => {
    await fetch(`/api/admin/tasbih-presets/${id}`, { method: 'DELETE' });
    fetchAdminData();
  };
...
const TasbihModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      fetch('/api/tasbih-presets').then(res => res.json()).then(data => {
        setPresets(data);
